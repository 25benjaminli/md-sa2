    ...
    for epoch in range(max_epochs):
        ...
        if (epoch+1) % val_every == 0:
            ...
            if json_metrics["dice"]["avg"] > best_dice:
                ...
                # save model
                runs_dir = generate_rndm_path(f"./runs/{config.model_type}_{fold_val}")
                unet_model.save_model(epoch=epoch, save_dir=runs_dir)
                with open(join(runs_dir, f"best_metrics_{config.model_type}_{fold_val}"), "w") as f:
...
if __name__ == "__main__":
    ...
    for fold_val in folds_to_test:
        print(f"---- training fold {fold_val} ----")
        single_fold(fold_val)