...
class Sam(nn.Module):
    ...
    def forward(self, batched_input, multimask_output, image_size, boxes=None):
        if isinstance(batched_input, list):
            print("forward sam", batched_input[0].keys())
            outputs = self.forward_test(batched_input, multimask_output)
        else:
            outputs = self.forward_train(batched_input, multimask_output, image_size, boxes=boxes)
        return outputs
    ...
    def forward_train(self, batched_input, multimask_output, image_size, points=None, boxes=None):
        ...
        # print("multimask output", multimask_output)
        ...
        # print("low res predictions", low_res_logits.shape)
