...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        try:
            ...
            # print(new_state_dict['image_encoder.blocks.11.attn.rel_pos_h'].shape)
            # print(new_state_dict['image_encoder.blocks.15.attn.rel_pos_h'].shape)
...
def load_from(sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes):
    ...
    if pos_embed.shape[1] != token_size:
        ...
        # print("new pos emb", pos_embed.shape)

        pos_embed = pos_embed.permute(0, 2, 3, 1)  # [b, h, w, c]
        new_state_dict['image_encoder.pos_embed'] = pos_embed
        rel_pos_keys = [k for k in sam_dict.keys() if 'rel_pos' in k]
        # '2' in k or '5' in k or '8' in k or '11' in k
        global_rel_pos_keys = [k for k in rel_pos_keys if any(str(att) in k for att in encoder_global_attn_indexes)]
