...
class AggregatorUNetWrapper():
    def __init__(self, train_loader, val_loader,
                  loss_func, scaler, optimizer,
                 config, **model_params):
        
        if config.model_type == "unet_basic":
    ...
    def validate_epoch(self, epoch):
        ...
        with torch.no_grad():
            for idx, batch_data in enumerate(self.val_loader):
                ...
                self.metric_accumulator.update(y_pred=val_output_convert, y_true=val_labels_list)
                ...
                if self.config.fold_val[0] in self.config.volumes_to_collect.keys():
                    if batch_data["image_title"][0] in self.config.volumes_to_collect[self.config.fold_val[0]]:
                        ...
                        np.save(os.path.join(predictions_base, f"{batch_data['image_title'][0]}_fold_{self.config.fold_val[0]}.npy"), val_output_convert[0])
